                ...
                is KSClassDeclaration    -> {
                    if (it.classKind == ClassKind.OBJECT) {
                        ...
                        if (ctors.isEmpty() || ctors.any { it.parameters.all { it.hasDefault } }) {
                            ...
                            isValid = false
                            logger.error(
                                "@ServiceProvider target class '${it.simpleName.asString()}' must have a defaultable constructor or be an 'object'",
                                it
                            )
                        }
                    } else {
                        isValid = false
                        logger.error(
                            "@ServiceProvider target '${it.simpleName.asString()}' should be an 'object' or 'class' with a defaultable constructor",
                            it
                        )
                    }
                }
